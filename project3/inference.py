...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        #    print(ghostPosition == jailPosition, jailPosition)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        myPosition = gameState.getPacmanPosition()
        probDict = util.Counter()

        #if observation == None:
            #probDict[self.getJailPosition()] = 0.0
        #else:
        for p in self.allPositions:
            dist = util.manhattanDistance(p, myPosition)
            probDict[p] = self.getObservationProb(observation, myPosition, p, self.getJailPosition()) * self.beliefs[p]

        self.beliefs = probDict
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldGPosition in self.allPositions:
            possPos = self.getPositionDistribution(gameState, oldGPosition)
            for pos, prob in possPos.items():
                probDict[pos] += prob * self.beliefs[oldGPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if observation == None:
            ...
            possiblePos = util.Counter()
            myBeliefs = self.getBeliefDistribution()
            for pos in self.legalPositions:
                dist = util.manhattanDistance(myPosition, pos)
                possiblePos[pos] += busters.getObservationProbability(observation, dist) * myBeliefs[pos]
            possiblePos.normalize()

            #Check if all values are 0
            flag = True
            for value in possiblePos.values():
                if value != 0:
                    flag = False 
                    break
            ...
            if flag:
                ...
                for i in range(0, self.numParticles):
                    resample.append(util.sample(possiblePos))
    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.particles:
            newPosDistr = self.getPositionDistribution(gameState, oldPosition)
            
            #print(newPosDistr.items)
            #print("Temp", temp)
            #print(type(newPosDistr))

            #Util.sample
            distribution = newPosDistr
