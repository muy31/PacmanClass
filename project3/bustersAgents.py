...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #Looks through living ghosts for each position of highest likelihood
        targetIdeas = []
        for distr in livingGhostPositionDistributions:
            targetIdeas.append(distr.argMax())

        #Chooses the ghost whose location is best known
        bestTarget = None
        bestProb = 0

        for targetIndex, pos in enumerate(targetIdeas):
            if livingGhostPositionDistributions[targetIndex][pos] > bestProb:
                bestTarget = pos
                bestProb = livingGhostPositionDistributions[targetIndex][pos]

        #Chooses the action that moves closest to targeted position
        bestAction = None
        closestDist = 999999999999
        for action in legal:
            nextLocation = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(nextLocation, bestTarget)
