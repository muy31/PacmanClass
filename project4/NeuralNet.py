...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(inActs)):
            newWt = self.weights[i] + alpha * delta*inActs[i]
            nextWeights.append(newWt)
            totalModification += nextWeights[i] - self.weights[i]

        self.weights = nextWeights

        if totalModification < 0: 
            totalModification *= -1.0
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            neuroList = []
            for neuron in layer:
                neuroList.append(neuron.sigmoidActivation(currentInput))
            currentInput = neuroList
            returnList.append(neuroList)
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    for nextNeuron in range(len(nextLayer)):
                        weightedDelta = nextLayer[nextNeuron].weights[neuronNum + 1] * deltas[0][nextNeuron]
                        nextLayerError = nextLayerError + weightedDelta
                    delta = gPrime * nextLayerError
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for testIn, testOutput in examplesTest:
        outputs = nnet.feedForward(testIn)[-1]
        match = True
        for val in range(len(outputs)):
            if round(outputs[val]) != testOutput[val]:
                match = False
        if match:
    ...
    testAccuracy = testCorrect / ((testCorrect + testError) * 1.0)
