...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        cornersLeft = [False, False, False, False]

        for i in range(0,4):
            if self.startingPosition == self.corners[i]:
                cornersLeft[i] = True

        #print(self.startingPosition, self.corners, cornersLeft)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            nxy = (int(xy[0] + dxy[0]), int(xy[1] + dxy[1]))


            if not self.walls[nxy[0]][nxy[1]]:
                cornersLeft = state[1].copy() #cornersLeft
                for i in range(0,4):
                    if nxy == self.corners[i]:
                        cornersLeft[i] = True
                successors.append( ((nxy, cornersLeft), action, 0) )
...
def cornersHeuristic(state, problem):
    ...
    #Calculate total distance from state to all corners (that haven't been reached yet)
    value = 0;
    
    for i in range(0, 4):
        if not state[1][i]:
            value += abs(state[0][0] - corners[i][0]) + abs(state[0][1] - corners[i][1])

    #Original idea was to deviate away from the center
    #startValue -= manhattanDistance(state, (startX/2, startY/2))
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        value += abs(food[0] - position[0]) + abs(food[1] - position[1])/(problem.walls.width * problem.walls.height)
